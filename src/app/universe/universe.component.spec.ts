...
describe('Universe', () => {
  ...
  it('Universe with 1 single active cell should evolve to a univers with no active cell', async(() => {
    var universe = new UniverseComponent();
    universe.generation.push(new Cell(0, 0, true)); 
    universe.evolve();

    let result = universe.indexOfPair(universe.generation, 0,0);
  ...
  it('Universe with disabled cell & 3 active neighbors should be reborned', async(() => {
    ...
    universe.generation.push(new Cell(0, 0, true));
    universe.generation.push(new Cell(0, 2, true));
    universe.generation.push(new Cell(2, 0, true));

    universe.evolve();

    let result = universe.indexOfPair(universe.generation, 1,1);
  ...
  it('Universe with active cell & 4 active neighbors should evolve to a univers with the active cell disabled', async(() => {
    ...
    universe.generation.push(new Cell(1, 1, true));
    universe.generation.push(new Cell(0, 0, true));
    universe.generation.push(new Cell(0, 2, true));
    universe.generation.push(new Cell(2, 0, true));
    universe.generation.push(new Cell(2, 2, true));
  ...
  it('Universe with active cell & 2 active neighbors should be kept alive', async(() => {
    ...
    let result1 = universe.indexOfPair(universe.generation, 1,1);
    let result2 = universe.indexOfPair(universe.generation, 1,0);
    let result3 = universe.indexOfPair(universe.generation, 0,0);
